	...
	\begin{itemize}
		...
		\item Динамические бинарные трансляторы с поднятием в LLVM IR:
		      \begin{itemize}
			      ...
			      \item DBILL, LnQ, Risotto (есть только статьи, либо код закрыт)
		      \end{itemize}
		      %    \item Выводы:
		      %          \begin{itemize}
...
\begin{frame}[fragile]
	\frametitle{Тестирование (1/2)}
	% \begin{itemize}
	%   \item В реализации интересны архитектура, библиотеки, инструменты
	%   \item Не надо добавлять на слайд примеры кода
	% \end{itemize}
	\begin{itemize}
		\item Для выявления ошибок и улучшения поддержки RISC-V-хоста
		\item Используем следующий подход:
		      ...
		      \begin{itemize}
			      ...
			      %        \item riscv64
			      %       \end{itemize}
			      \item Выбираем набор тестовых программ
			      \item Компилируем статически и динамически
			      \item В том числе компилируем под разные платформы
			      \item Выявляем ошибки и исправляем их
		      \end{itemize}
		\item В процессе тестирования было сделано:
		      \begin{itemize}
			      \item Добавлены новые релокации и обновлена PLT-таблица
			      \item Сделан рефакторинг
			            % \item Написаны скрипты для запуска динамически слинкованных программ
		      \end{itemize}
		\item Из трудностей можно выделить:
		      \begin{itemize}
			      \item Частично тестирование проводилось на плате Sipeed Lichee PI 4A, где был установлен специфический glibc, в следствие чего было потрачено время на поиск плохо задокументированных инструкций, которые Rellume не мог поднять в LLVM IR
		      \end{itemize}
	\end{itemize}
	% \vspace{-8pt}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item Instrew использует Rellume в качестве git submodule
		\item При запуске Instrew unit-тестов также запускаются unit-тесты Rellume
		      \begin{itemize}
	...
	\begin{itemize}
		...
		\item Есть два варианта решения этой проблемы:
		      \begin{itemize}
			      ...
			      \item C помощью qemu-user и chroot
		      \end{itemize}
		\item Удалось опробовать оба решения, но был выбран второй подход, так как:
		      \begin{itemize}
			      \item Есть сложности с поддержкой self-hosted runner на RISC-V
...
\begin{frame}
	...
	\begin{enumerate}
		...
		\item В CI добавлена поддержка сборки на RISC-V
