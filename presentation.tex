...
\begin{frame}
	...
	{\scriptsize
		...
		\textbf{Научный руководитель:} к.ф.-м.н. Д.В. Луцив, доцент кафедры системного программирования \\
		\textbf{Консультант:} В.А. Кутуев, инженер-исследователь, Лаборатория технологий программирования инфраструктурных
		решений СПбГУ
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item Важным классом которых являются динамические бинарные трансляторы (далее --- ДБТ)
		      \begin{itemize}
			      \item Используются для эмуляции "на лету"
			      \item Например QEMU-user, Rosetta 2, Box64, Valgrind
		      \end{itemize}
		\item В качестве промежуточного представления для ДБТ хорошо использовать LLVM IR
		      ...
		      \end{itemize}
		\item Хотим ДБТ с поднятием LLVM IR для RISC-V
	...
	\begin{itemize}
		...
		\item Динамические бинарные трансляторы с поднятием в LLVM IR:
		      \begin{itemize}
			      ...
			      \item BinRec, HQEMU
			            \begin{itemize}
				            \item Не обновлялись с 2022 и 2018 года соответственно
			            \end{itemize}
			      \item DBILL, LnQ, Risotto
			            \begin{itemize}
				            \item Есть только статьи, либо код закрыт
			            \end{itemize}
		      \end{itemize}
		      %    \item Выводы:
		      %          \begin{itemize}
	...
	\begin{itemize}
		\item Выводы:
		      \begin{itemize}
			      ...
			      \item Instrew имеет API для добавления инструментов бинарного анализа
			      \item Является open-source проектом
			      \item Из всех рассмотренных ДБТ с поднятием в LLVM IR, Instrew + Rellume единственный поддерживается мейнтейнером
...
% \begin{frame}[fragile]
% 	\frametitle{Обзор}
% 	В предыдущей работе было сделано:
% 	\begin{itemize}
...
% 		\item Реализована функциональность для запуска статически слинкованных программ с простым рантаймом
% 	\end{itemize}
% \end{frame}
...
\begin{frame}
	...
	\begin{enumerate}
		...
		%       \end{itemize}
		% \item на 7-ой семестр:
		% \begin{itemize}
		\item Протестировать на произвольных статически слинкованных программах
		\item Исправить ошибки, выявленные при тестировании
		\item Добавить в CI поддержку сборки на RISC-V
		      % \end{itemize}
	\end{enumerate}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item Работа была проведена над
		      \begin{itemize}
			      \item "ELF Linker" в клиенте Instrew
			      \item Rellume в сервере Instrew
		      \end{itemize}
	\end{itemize}
	\begin{figure}
...
\begin{frame}[fragile]
	\frametitle{Тестирование Instrew}
	% \begin{itemize}
	%   \item В реализации интересны архитектура, библиотеки, инструменты
	%   \item Не надо добавлять на слайд примеры кода
	% \end{itemize}
	\begin{itemize}
		\item Используем coreutils в качестве набора тестовых программ
		      % \begin{itemize}
		      %  \item статически и динамически слинкованные программы из coreutils под:
		      %        %       \begin{itemize}
		      %        %        \item x86\_64
		      %        %        \item riscv64
		      %        %       \end{itemize}
		      %        % \item Необходимо, так как если Instrew не работает на coreutils, то не работает вообще
		      %        % \item Достаточно, так как <причина>
		      % \end{itemize}
		      % \item Используем 4 вариации coreutils
		      % \begin{itemize}
		      %  \item Статически и динамически слинкованные
		      %  \item Скомпилированные под x86\_64 и riscv64
		      % \end{itemize}
		...
		\item Результат
		      \begin{center}
			      ...
			      \begin{tabular}{ |p{3cm}|p{3cm}|p{3cm}|}
				      \hline
				      Guest Архитектура& Статический Coreutils &Динамический Сoreutils\\
				      \hline
				      rv64  & проходят K из P тестов &проходят L из P тестов\\
				      x86\_64& проходят M из P тестов  & проходят N из P тестов \\
				      \hline
			      \end{tabular}
		      \end{center}
...
\begin{frame}[fragile]
	\frametitle{Исправленные ошибки в Instrew}
	\begin{itemize}
		\item В процессе тестирования были обнаружены ошибки:
		      \begin{itemize}
			      \item Отсутсвуют N релокаций
			      \item Добавлено N новых релокаций
			      \item в PLT-таблицу добавлено M вхождений
			      \item Добавлен latomic к зависимостям
			            % \item Написаны скрипты для запуска динамически слинкованных программ
		      \end{itemize}
	\end{itemize}
\end{frame}

\begin{frame}[fragile]
	\frametitle{Проблемы Instrew}
	% Из трудностей можно выделить:
	\begin{itemize}
		\item Тестирование <частично> проводилось на плате Sipeed Lichee PI 4A
		\item *Однако подобный тест-кейс на плате делать неудобно, потому что*
		      \begin{itemize}
			      \item Установлен glibc с нестандартными расширениями
			      \item Rellume не мог поднять в LLVM IR инструкции из этого расширения
		      \end{itemize}
		\item Поиск документации для инструкций из этого расширения занял много времени
		      \begin{itemize}
			      \item Было принято решение проводить тестирование на виртуальной машине
		      \end{itemize}
	\end{itemize}
\end{frame}

\begin{frame}[fragile]
	\frametitle{Тестирование Rellume}
	% \begin{itemize}
	%   \item В реализации интересны архитектура, библиотеки, инструменты
	%   \item Не надо добавлять на слайд примеры кода
	% \end{itemize}
	\begin{itemize}
		% \item В процессе тестирования оказалось, что в Rellume плохая поддержка RISC-V-хоста
		\item Некоторые внутренние тест-кейсы используют LLVM JIT, которые падают с SEGFAULT на RISC-V
		      \begin{itemize}
			      \item В следствие чего снижается покрытие тестами
		      \end{itemize}
		      % \item Для CI важно, чтобы все тесты проходили, хотим решить эту проблему
		\item Попробовали обновить версию LLVM до 21, что потенциально может решить проблему
		      % \begin{itemize}
		      %  \item Кросс-компилируем LLVM trunk под RISC-V
		      %  \item Делаем рефакторинг Rellume
		      %        % \item Оказывается, что при обновлении дополнительно перестает работать MCAssembler, который также падает с SEGFAULT, тем самым уменьшая количество успешно пройденных тестов
		      % \end{itemize}
		\item Результат
		      \begin{itemize}
			      \item Все также падают тесты в рантайме
			      \item Но знаем номера недостающих релокаций
			            % \item Можно сделать патч в LLVM
		      \end{itemize}
...
\begin{frame}[fragile]
	\frametitle{CI}
	\begin{itemize}
		\item Есть трудности в связи с отсутствием нативной поддержки RISC-V в GH Actions
		\item Есть два варианта решения этой проблемы:
		      \begin{itemize}
			      \item C помощью self-hosted runner на RISC-V
			      \item C помощью эмуляции RISC-V (например с помощью qemu-user)
		      \end{itemize}
		\item Удалось опробовать оба решения, но был выбран второй подход, так как:
		      \begin{itemize}
			      % \item Не удалось запустить runner на виртуальной машине
			      \item Self-hosted runner неудобен для мейнтейнера
			            % \item Удалось найти third-party action, с помощью которого не нужно настраивать qemu-user и chroot
			      \item Actions runner использует .NET, который на данный момент плохо поддерживает RISC-V
			            \begin{itemize}
				            \item Не удалось запустить на виртуальной машине, только на плате
			            \end{itemize}
		      \end{itemize}
		      % \item Также после добавления CI был сделан PR в основную ветку Instrew
	\end{itemize}
	% \vspace{-8pt}
\end{frame}
%
...
\begin{frame}
	...
	\vspace{8pt}
	\begin{enumerate}
		% \item Instrew был протестирован на произвольных статически слинкованных программах
		% \item Исправлены некоторые ошибки, выявленные при тестировании
		\item Instrew был протестирован на 4-ёх вариациях coreutils
		\item Добавлено N новых релокаций, M вхождений в PLT-таблицу; добавлен latomic к зависимостям
		\item В CI добавлена поддержка сборки на RISC-V и сделан пулл-реквест в основную ветку Instrew
	...
	\end{enumerate}
	\vspace{8pt}
	Код доступен по данной ссылке: \href{https://github.com/aengelke/instrew/pull/20}{\color{blue}{https://github.com/aengelke/instrew/pull/20}}.
...
% \begin{frame}[t]
% 	\frametitle{glibc}
% 	\includegraphics[width=10cm]{pictures/glibc.jpg}
% \end{frame}
% \begin{frame}
