...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item Используем 4 вариации coreutils
		      ...
		      % \end{itemize}
		\item В процессе тестирования было сделано:
		      \begin{itemize}
		...
		\item Результат
		      \begin{center}
			      ...
			      \begin{tabular}{ |p{3cm}|p{3cm}|p{3cm}|}
				      \hline
				      Архитектура& Статический Coreutils &Динамический Сoreutils\\
				      \hline
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item Некоторые тест-кейсы используют LLVM JIT, которые падают с SEGFAULT на RISC-V
		      \begin{itemize}
			      \item В следствие чего снижается покрытие тестами
		      \end{itemize}
		      % \item Для CI важно, чтобы все тесты проходили, хотим решить эту проблему
		\item Хотим попробовать обновить версию LLVM до 21, что потенциально может решить проблему
		      \begin{itemize}
		...
		\item Результат
		      \begin{itemize}
			      \item Все также падают тесты в рантайме, но знаем номер нереализованной релокации
	...
	\begin{itemize}
		...
		\item Есть два варианта решения этой проблемы:
		      \begin{itemize}
			      ...
			      \item C помощью эмуляции RISC-V
		      \end{itemize}
		\item Удалось опробовать оба решения, но был выбран второй подход, так как:
		      \begin{itemize}
			      ...
			      \item Actions runner использует .NET, который на данный момент плохо поддерживает RISC-V
			            \begin{itemize}
				            \item Не удалось запустить на виртуальной машине, только на платке
			            \end{itemize}
		      \end{itemize}
